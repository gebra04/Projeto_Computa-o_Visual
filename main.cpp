...
const float size = 10.0f; // Tamanho do grid (ex.: 10 unidades em cada direção)
const float step = 1.0f;  // Espaçamento entre linhas
const int numLines = (int)(size / step) * 2 + 1; // Número total de linhas
...
void display() {
  ...
  // Translation. 
	//glm::mat4 T = glm::translate(glm::mat4(1.0f), glm::vec3(x, 0.0f, 0.0f));
  ...
  glm::mat4 Ry;
  glm::mat4 Rz;
  Rx = glm::rotate(glm::mat4(1.0f), glm::radians(angle), glm::vec3(1.0f,0.0f,0.0f));
  Ry = glm::rotate(glm::mat4(1.0f), glm::radians(angle), glm::vec3(0.0f,1.0f,0.0f));
  Rz = glm::rotate(glm::mat4(1.0f), glm::radians(angle), glm::vec3(0.0f,0.0f,1.0f));
  
	if(cond){
    T = glm::rotate(glm::mat4(1.0f), glm::radians(angle), glm::vec3(0.0f,0.0f,1.0f));
	}else{
    T = glm::translate(glm::mat4(1.0f), glm::vec3(x, 0.0f, 0.0f));
	}
...
void idle() {
  ...
  //   	//angle = ((angle+angle_inc) < 180.0f) ? angle+angle_inc : 180.0-angle+angle_inc;
	// 	angle += angle_inc;
	// 	if(angle >= 179.5f && angle <= 180.0f){
	// 		angle = 180.0f;
	// 		cond = 0;
	// 		x += x_inc;
	// 	}else if(angle >= 359.5f && angle <= 360.0f){
	// 		angle = 0.0f;
	// 		cond = 0;
	// 		x += x_inc;
	// 	}
